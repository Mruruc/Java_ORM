...
public class GenericRepository<T, ID> {
    ...
    // the merge check if the entity has id field it will update the state in persistence context.
    // if the entity does not have id field it will insert the entity.
    ...
    public void delete(ID id) {
        ...
        try (EntityManager entityManager = JPAUtil.getEmf().createEntityManager()) {
            transaction = entityManager.getTransaction();
            transaction.begin();
            T entityFound = Optional.ofNullable(entityManager.find(entityClazz, id))
